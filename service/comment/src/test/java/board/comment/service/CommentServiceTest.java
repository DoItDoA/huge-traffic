...
class CommentServiceTest {
    ...
    void deleteShouldMarkDeletedIfHasChildren() {
        ...
        given(commentRepository.countBy(articleId, commentId, 2L)).willReturn(2L); // 출력이 2L이 나오게 설정
    ...
    void deleteShouldDeleteChildOnlyIfNotDeletedParent() {
        ...
        given(comment.isRoot()).willReturn(false); // isRoot를 false가 나오게 설정

        Comment parentComment = mock(Comment.class);
        given(parentComment.getDeleted()).willReturn(false); // 부모 댓글이 false가 나오게 설정
